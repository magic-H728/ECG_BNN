...
def get_data(classes_num, device, test_size):
    ...
    if classes_num == 17:
        ...
        for root, dirs, files in os.walk(dataset_path, topdown=False):
            for name in files:
                data_train = np.load(os.path.join(root, name))  # 取出字典里的value
...
class Loader:
    def __init__(self, batch_size, classes_num, device, test_size):
        self.labels, self.x_train, self.x_test, self.y_train, self.y_test = get_data(
            classes_num, device, test_size
        )
